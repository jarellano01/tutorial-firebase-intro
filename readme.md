...
In this tutorial we will be building a very simple chatroom to demonstrate the quick and easy database functionality that Google's Firebase has to offer.

After this tutorial you will have learned how to do the following:
- create a new Firebase Project
- Initialize your Firebase database with in your web app
- Pull snapshots of your database when ever a value changes
...
Fortunutely, Firebase is quite simple not only to use but also to set up. 
...
We will not be going over auth in this tutorial and therefore will be required to all full read/write access. Please note that it is never a good idea to leave a database open to full read/write access. 
...
One of my favorite things about firebase, is the fact that it very simple to initialize and to get started. 
...
The On Value change function will run once when your app is initiated as well as when ever data changes in your database. 

For example let's say that some one across the world decided to send a message in our amazing chatroom app, Firebase will hear that change and therefore will run what ever callback we send through the On Value change function. 

In the example below, I have create a variable called chatroom which will reference the item in our database which holds messages for our chatroom. In this case our chatroom is called 'ChatroomForHackers'. 
```
...
Now I can attach an "on value change" listener to my chatroom variable. Therefore when ever a message is added to this chatroom, Firebase will shoot us over a snapshot of all the data as a JSON object which we can then process as we wish. In this case I am simply logging it to our console. 
...
Currently the unsolved code works with an object, `sampleMessagesObject`, that has been hard coded inside of `js/firebase.js`. Any time a message is "sent", this object is updated, and then we run `updateUI(sampleMessagesObject)` to re-render the messages in our chatroom. 
...
Firstly, we update the UI with messages already in Firebase when we first launch our app. 
    
Secondly, when we push an item into our Firebase database, Firebase hears the change which then triggers an update to our UI since we set an 'on value change' listener. 
